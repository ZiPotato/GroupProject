...
Currently instead of using the actual API:s, we simulate the Json data that we would get from them locally (Once we actually merge the published version). Using the Json data we create Parcel records that we use to display the current state of the package.
